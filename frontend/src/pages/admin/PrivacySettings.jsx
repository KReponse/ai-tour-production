// src/pages/admin/PrivacySettings.jsx
// ✅ NEW - Privacy Policy Settings Admin Page
...
import { getPrivacyContent, updatePrivacyContent, resetPrivacyContent } from '../../services/privacyService';
...
const PrivacySettings = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      toast.error('Failed to load privacy content');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSave = async () => {
    try {
      setSaving(true);
      await updatePrivacyContent(data, token);
  ...
  const handleReset = async () => {
    if (window.confirm('Reset privacy content to defaults? This cannot be undone.')) {
      try {
        await resetPrivacyContent(token);
        await fetchData();
        toast.success('Privacy content reset to defaults');
      } catch (error) {
        console.error('Error resetting privacy:', error);
        toast.error('Failed to reset privacy content');
      }
    }
  };
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh]">
        ...
        <p className="mt-4 text-gray-500 dark:text-gray-400">Loading privacy settings...</p>
      </div>
    );
  }
  ...
  if (!data) {
    return (
      <div className="bg-white dark:bg-gray-900 rounded-3xl p-12 text-center border border-gray-200 dark:border-gray-800">
        ...
        <h2 className="text-2xl font-bold text-[#374151] dark:text-white">Failed to load privacy data</h2>
        <button
          onClick={fetchData}
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-8">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-black text-[#374151] dark:text-white">Privacy Policy Settings</h1>
          <p className="text-gray-500 dark:text-gray-400 mt-1">Manage your Privacy Policy content</p>
        </div>
