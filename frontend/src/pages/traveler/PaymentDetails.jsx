// frontend/src/pages/admin/PaymentDetails.jsx
// ✅ ADMIN PAYMENT DETAILS PAGE

import React, { useState, useEffect } from 'react';
import { useParams, useNavigate } from 'react-router-dom';
import { ArrowLeft, CreditCard, Loader2, Sparkles, Calendar, DollarSign, User, Building2, CheckCircle, XCircle, Clock, AlertCircle, Receipt, Shield, TrendingUp, Download } from 'lucide-react';
import { getPaymentById, processRefund, downloadReceipt } from '../../services/paymentService';
...
const AdminPaymentDetails = () => {
  ...
  if (error || !payment) {
    return (
      <div className="flex flex-col items-center justify-center h-96 text-center">
        ...
        <button onClick={() => navigate('/admin/payments')} className="mt-4 px-6 py-2 rounded-xl bg-[#0D9488] text-white font-bold hover:bg-[#0D9488]/80 transition">Back to Payments</button>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6 max-w-4xl mx-auto px-4 py-6">
      <button onClick={() => navigate('/admin/payments')} className="flex items-center gap-2 text-gray-500 hover:text-[#0D9488] transition">
        <ArrowLeft className="w-4 h-4" /> Back to Payments
      </button>

      <div className="flex items-center justify-between flex-wrap gap-4">
        <div className="flex items-center gap-3">
          <div className="w-12 h-12 rounded-2xl bg-gradient-to-br from-[#0D9488] to-[#F59E0B] flex items-center justify-center shadow-lg"><Shield className="w-6 h-6 text-white" /></div>
        ...
        <div className="flex items-center gap-3">
          {payment.status === 'paid' && (
            <>
              <button onClick={handleDownloadReceipt} className="px-4 py-2 rounded-xl bg-[#0D9488] text-white font-medium hover:bg-[#0D9488]/80 transition flex items-center gap-2"><Download className="w-4 h-4" /> Receipt</button>
              <button onClick={handleProcessRefund} disabled={processing} className="px-4 py-2 rounded-xl bg-red-500 text-white font-medium hover:bg-red-600 transition disabled:opacity-50 flex items-center gap-2">
                {processing ? <Loader2 className="w-4 h-4 animate-spin" /> : 'Process Refund'}
              </button>
            </>
          )}
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="bg-white dark:bg-gray-900 rounded-3xl border border-gray-200 dark:border-gray-800 p-6 shadow-sm">
          <h2 className="text-lg font-bold text-[#374151] dark:text-white mb-4 flex items-center gap-2"><User className="w-5 h-5 text-[#0D9488]" /> Traveler</h2>
          <div className="space-y-3">
            <div><p className="text-sm text-gray-500">Name</p><p className="font-semibold text-[#374151] dark:text-white">{payment.user?.name || 'N/A'}</p></div>
            <div><p className="text-sm text-gray-500">Email</p><p className="font-semibold text-[#374151] dark:text-white">{payment.user?.email || 'N/A'}</p></div>
          </div>
        </div>

        <div className="bg-white dark:bg-gray-900 rounded-3xl border border-gray-200 dark:border-gray-800 p-6 shadow-sm">
          <h2 className="text-lg font-bold text-[#374151] dark:text-white mb-4 flex items-center gap-2"><Building2 className="w-5 h-5 text-[#0D9488]" /> Provider</h2>
          <div className="space-y-3">
            <div><p className="text-sm text-gray-500">Name</p><p className="font-semibold text-[#374151] dark:text-white">{payment.provider?.businessName || payment.provider?.name || 'N/A'}</p></div>
            <div><p className="text-sm text-gray-500">Email</p><p className="font-semibold text-[#374151] dark:text-white">{payment.provider?.email || 'N/A'}</p></div>
          </div>
        </div>
      </div>
