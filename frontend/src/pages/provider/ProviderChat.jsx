// frontend/src/pages/provider/ProviderChat.jsx
// ✅ NEW - Provider Chat Interface with Travelers + Support
...
const ProviderChat = () => {
  ...
  const selectConversation = useCallback(async (conversation) => {
    ...
    navigate(`/provider/chat/${conversation._id}`, { replace: true });
  ...
  const handleSupportChat = async () => {
    try {
      const response = await createSupportConversation(
        'Hello, I need support with my account or listings.',
        'provider_support'
      );
      if (response.success) {
          ...
          {filteredConversations.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-48 text-center p-4">
              ...
              <p className="text-sm text-gray-400 dark:text-gray-500 mt-1">
                {searchTerm ? 'Try a different search term' : 'Start chatting with travelers'}
              </p>
            </div>
          ) : (
            filteredConversations.map((conv) => {
              ...
              const name = getConversationPartnerName(conv, user?._id);
              const role = getConversationPartnerRole(conv, user?._id);
