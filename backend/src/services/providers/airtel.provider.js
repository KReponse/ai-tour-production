// backend/src/services/providers/airtel.provider.js
// ✅ NEW - Airtel Money Provider (Placeholder)
 ...
 * 
 * Implements the PaymentProviderInterface for Airtel Money.
 ...
 * - AIRTEL_API_SECRET
 * - AIRTEL_ENVIRONMENT
 * - AIRTEL_COLLECTION_ACCOUNT
 * - AIRTEL_CALLBACK_URL
 * 
 * Reference: https://developers.airtel.africa/
 */
class AirtelProvider extends PaymentProviderInterface {
  constructor(config = {}) {
    super(config);

    // ─── Provider Identity ──────────────────────────────────────
    this._providerId = 'airtel';
    this._providerName = 'Airtel Money';
    ...
    this.apiSecret = config.apiSecret || process.env.AIRTEL_API_SECRET;
    this.environment = config.environment || process.env.AIRTEL_ENVIRONMENT || 'sandbox';
    this.collectionAccount = config.collectionAccount || process.env.AIRTEL_COLLECTION_ACCOUNT;
    this.callbackUrl = config.callbackUrl || process.env.AIRTEL_CALLBACK_URL;

    // ─── API Base URLs ──────────────────────────────────────────
    this.apiBaseUrl = this.environment === 'production'
      ? 'https://api.airtel.com/merchant/v1'
      : 'https://sandbox.airtel.com/merchant/v1';

    console.log(`📱 Airtel Money Provider initialized (${this.environment} mode)`);
  ...
  async createPayment(data) {
    try {
      ...
      if (!this.isEnabled) {
        throw new Error('Airtel Money provider is not configured. Please set API credentials.');
      }
      ...
      if (!this.supportedCurrencies.includes(currency)) {
        throw new Error(`Currency "${currency}" is not supported by Airtel Money`);
      }

      // ─── Generate payment reference ─────────────────────────────
      const paymentReference = this.generateReference('AIRTEL');
      ...
      const paymentData = {
        ...
        callbackUrl: data.webhookUrl || this.callbackUrl || `${process.env.API_URL}/api/payments/webhook/airtel`,
      };
      ...
      // ─── TODO: Call Airtel Money API to request payment ────────
      ...
      return {
        ...
        paymentUrl: null, // Airtel uses USSD/push notification
        metadata: {
          ...
          instructions: 'You will receive a payment request on your Airtel Money. Please approve it to complete payment.',
        },
      };
  ...
  /**
   * Verify an Airtel Money payment
   */
  async verifyPayment(paymentId, options = {}) {
    try {
      ...
      // ─── TODO: Call Airtel Money API to get payment status ────
  ...
  async refundPayment(paymentId, amount = null, reason = '', options = {}) {
    try {
      ...
      // ─── TODO: Call Airtel Money refund/disbursement API ──────
  ...
  async verifyWebhookSignature(req, signature, secret) {
    try {
      // ─── Airtel uses HMAC-SHA256 for signatures ──────────────
  ...
  /**
   * Format amount for Airtel Money
   */
  ...
  async requestPayment(paymentData) {
    // TODO: Implement actual API call to Airtel Money
    // const headers = {
    //   'X-Airtel-Api-Key': this.apiKey,
    //   'X-Airtel-Api-Secret': this.apiSecret,
  ...
  /**
   * Get payment status from Airtel Money API
   */
  ...
  /**
   * Request refund from Airtel Money API
   */
  ...
  /**
   * Format payment response from Airtel Money API
   */
  ...
  /**
   * Map Airtel Money API status to our status
   */
  ...
  /**
   * Handle provider error with Airtel Money specific error codes
   */
  handleError(error, context = '') {
    ...
    if (error.code === 'AUTHENTICATION_FAILED') {
      ...
      errorMessage = 'Airtel Money authentication failed. Please check API credentials.';
